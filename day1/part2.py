...
amended by a very young Elf who was apparently just excited to show off her art skills. Consequently, the Elves are having
...
def checkForNum(line, index) -> int:
    if line[index].isnumeric():
        return int(line[index])

    if index + 3 < len(line) and line[index:index+3] in numMappings:
        return numMappings[line[index:index+3]]
    elif index + 4 < len(line) and line[index:index+4] in numMappings:
        return numMappings[line[index:index+4]]
    elif index + 5 < len(line) and line[index:index+5] in numMappings:
        return numMappings[line[index:index+5]]
...
def main():
    ...
    for line in lines:
        ...
        for i in range(0, lineLen):
            num = checkForNum(line, i)
            if num != -1:
                lineNums.append(num)

        calibration = (lineNums[0]*10)+lineNums[len(lineNums)-1]
        numbers.append(calibration)
        total += calibration


        print(f'found num: {calibration}, and total: {total}, from string: {line}')
