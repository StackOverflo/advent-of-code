...
def isGear(row, col, schematic):
    if row < 0 or row >= len(schematic) or col < 0 or col >= len(schematic[0]):
        return False
    return schematic[row][col] == '*'


def setGearIndexes(row, col, schematic, gearIndexes):
    for i in range(row-1,row+2):
        for j in range(col-1,col+2):
            if isGear(i,j, schematic):
                gearIndexes[f'{i},{j}'] = True
...
def main():
        ...
        while i < len(row):
            if row[i].isnumeric():
                ...
                for digitIndex in range(i, numEnd):
                    setGearIndexes(rowIndex, digitIndex, schematic, gearIndexes)

                for key in gearIndexes:
                    if key in gearMap:
    ...
    for key in gearMap:
        if len(gearMap[key]) == 2:
            result += gearMap[key][0]*gearMap[key][1]
