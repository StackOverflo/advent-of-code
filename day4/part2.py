...
, after all! I bet the gardener would know. He's on a different island, though - er, the small kind surrounded by
water, not the floating kind. We really need to come up with a better naming scheme. Tell you what: if you can help
...
def main():
    ...
    cardCopies = [0] * (len(lines) + 1)
    for line in lines:
        pipeSplit = line.split(' | ')
        winningNums = pipeSplit[0].split(': ')[1]
        scratchedNums = pipeSplit[1].strip('\n').split()
        cardNumber = int(re.findall(r'Card *(\d+):', line)[0])
        ...
        gamePoints = 0
        for num in scratchedNums:
            if num.isnumeric() and num in winningSet:
                gamePoints += 1

        if gamePoints > 0:
            for i in range(cardNumber + 1, cardNumber + gamePoints + 1):
                cardCopies[i] += cardCopies[cardNumber] + 1

    print(f'total points for all cards is {sum(cardCopies) + len(cardCopies) - 1}')
