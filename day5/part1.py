...
class Mapping:
    ...
    def sourceToDest(self, num):
        for entry in self.mapping:
            if num >= entry.sourceMinRange and num <= entry.sourceMaxRange:
...
def sectionToRanges(section):
    ...
    for i in range(0, len(section)):
        ...
        result.add(RangesEntry(source, dest, mapRange))
    return result
...
def main():
    ...
    for seed in seeds:
        cur = int(seed)
        cur = seedToSoil.sourceToDest(cur)
        cur = soilToFert.sourceToDest(cur)
        cur = fertToWater.sourceToDest(cur)
        cur = waterToLight.sourceToDest(cur)
        cur = lightToTemp.sourceToDest(cur)
        cur = tempToHum.sourceToDest(cur)
        cur = humToLoc.sourceToDest(cur)
