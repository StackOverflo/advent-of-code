...
def main():
    ...
    for line in lines:
        ...
        reds = getMax(re.findall(r"(\d+) red", line))
        greens = getMax(re.findall(r"(\d+) green", line))
        blues = getMax(re.findall(r"(\d+) blue", line))

        print(f'game: {game} red:{reds} greens:{greens} blues:{blues}')
        if reds <= totalReds and greens <= totalGreens and blues <= totalBlues:
            gameTotal += int(game)

    print(f'game num total: {gameTotal}')
