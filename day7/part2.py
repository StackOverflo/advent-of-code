...
def getHandRank(hand) -> int:
    ...
    # for J as wildcard, the highest hand would be changing the J to the highest frequency card
    if 'J' in cardFreq and len(cardFreq) > 1:
        added = cardFreq['J']
        del cardFreq['J']
        maxKey = max(cardFreq, key=cardFreq.get)
        cardFreq[maxKey] = cardFreq.get(maxKey, 0) + added
